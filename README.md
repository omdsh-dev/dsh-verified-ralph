...
Pull requests and `main` pushes run the complete Ubuntu/Windows matrix on Node 24 and the exact minimum Node 22.19.0, plus exact-commit Git-install and real Web/Headless profile-install smokes on Node 24. The profile smoke installs both plugins into the published DSH CLI, checks peers, verifies the official `ralph` and `verified_ralph` rows coexist, and boots each surface through a clean help exit. A source-contract gate pins audited DSH `dsh-v0.1.2-alpha.2` (`0a53fb55bea101816fa226bb964ae2bed71c343b`) and verifies the one-shot subagent, child token-limit, usage, and SessionEvent seams. CI remains keyless.
