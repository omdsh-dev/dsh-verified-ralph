...
插件不修改 DSH core，也不重新定义 verifier API。可复现构建依赖固定到 `dsh-as-a-verifier` merge commit `d74f80deb2de5b71004c4e81ed7c094eda663de0`（release `v0.2.4`）；运行时要求 verifier protocol 1 与离线 progress tracking。底层 progress 方法源自 llm-as-a-verifier（<https://github.com/llm-as-a-verifier/llm-as-a-verifier>）的 commit `8db8a114355a9d7fdf9a8d1d5c87f6aeebd18770`。归属见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
...
所有 PR 与 `main` push 都会在 Ubuntu/Windows 上以 Node 24 和精确最低版本 Node 22.19.0 运行完整矩阵，并以 Node 24 执行精确 commit Git-install smoke。源码合同门禁固定到已审查的 DSH `dsh-v0.1.2-alpha.1`（`cd5ef8148158c3a752a658978873241fdf8e2bbc`），验证 verifier-gated 流程使用的 one-shot subagent 与 SessionEvent seam。所有 Action 使用审查过的完整 SHA，并采用 Node 24 action runtime；CI 不读取 DeepSeek key，也不调用真实 API。稳定的 `required` check 是 `main` 门禁。
